...
def call_ollama(prompt):
    ...
    # payload = {"model": "llama3.1:8b", "prompt": prompt, "stream": False, "options": {"temperature": 0.2}}
...
def is_scanned_pdf(doc, threshold=0.2):
    ...
    for page in doc:
        if len(page.get_text().strip()) > 50:
            text_pages += 1
    return (text_pages / total_pages) < threshold
...
def handle_pdf_upload(uploaded_file):
    ...
    with st.spinner("Processing PDF..."):
        ...
        if not text.strip():
            st.error("Could not extract any structural text content from this file.")
            return
